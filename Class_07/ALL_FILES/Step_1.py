...
print(len(csv_files))
csv_files_glob = glob.glob("*.csv")
print(len(csv_files_glob))
...
files = list(filter(os.path.isdir, os.listdir(os.curdir)))  # files only, might not find anything
print(files)
...
list_s_folders = [folder for folder in list_s_folders if "S_" in folder]
...
#
# for folder in files:
#     if folder.startswith("S_"): #not found anything because there is no csv file from the prev directory
#         print(folder)
#         list_s_folders.append(folder)
# print(len(list_s_folders))
