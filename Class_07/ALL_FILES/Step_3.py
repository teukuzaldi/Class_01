...
# Task 1 - Create an in_memory shapefile with the locations,
# points_list =[[20.000,43.000],[25.500, 45.085],[26.574, 46.025], [28.131, 48.124]]
...
#2. add fields
arcpy.AddField_management("in_memory/points1", "Locations", "TEXT", field_length=20)
arcpy.AddField_management("in_memory/points1", "Species", "TEXT", field_length=20)

#3. Add data to each field
cur = arcpy.UpdateCursor("in_memory/points1")
for row in cur:
    row.setValue('Locations', "World")
    row.setValue('Species', "Phragmites")
    cur.updateRow(row)

#4. save to an actual output shapefile
arcpy.CopyFeatures_management("in_memory/points1", os.path.join(directory,"test.shp"))
