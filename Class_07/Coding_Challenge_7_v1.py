...
for species in speciesList:
    point_out_feature_class = (os.path.join(Temp_dir, f"{species}_point.shp"))
    ...
    if arcpy.Exists(point_out_feature_class):
        print(f"{species} point data is created successfully!")
...
# 3. Generate a fishnet,
        ...
        outFeatureClassNet = (os.path.join(Temp_dir, f"{species}_fishnet.shp"))  # Name of output fishnet
        ...
        if arcpy.Exists(outFeatureClassNet):
            print(f"{species} fishnet is created successfully!")
#
# 4. Undertake a Spatial Join to join the fishnet to the observed points.
        target_features = outFeatureClassNet
        join_features = saved_Layer
        out_feature_class = (os.path.join(Result_output, f"{species}_output.shp"))
        ...
        if arcpy.Exists(out_feature_class):
            print(f"{species} final output is created successfully!")
#
